...
class TaskContext:
    ...
    def report(
        ...
        **fields: Any,
        ...
        report_payload = _stringify_paths(dict(payload))
        report_path = self.write_json(report_payload, filename=filename, path=path)
        result = self.result(status=status or str(report_payload.get("status", "completed")))
        result[path_field] = str(report_path)

        if merge:
            skipped = {"status", *(exclude or ())}
        ...
        if include:
            for key in include:
                if key in report_payload:
                    result[key] = report_payload[key]

        result.update(_stringify_paths(fields))
        return result
