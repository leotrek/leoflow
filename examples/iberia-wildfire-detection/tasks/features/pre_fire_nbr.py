...
@task("features", name="pre_fire_nbr")
def main(ctx):
    workspace_root = Path(ctx.input_dir)
    nir, profile = read_raster(workspace_band(workspace_root, "pre_fire", "nir"))
    swir22, _ = read_raster(workspace_band(workspace_root, "pre_fire", "swir22"))
